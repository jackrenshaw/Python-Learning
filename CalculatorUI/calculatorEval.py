...
def Eval(string):
    ...
    numbersIdentified = list(re.findall(r"(([^0-9]\-)?[0-9]+(\.[0-9]+)?)", string)) #Find all numbers, accounting for negatives
...
def isValidExpression(string):
    if re.match(r"\"?(\-?[0-9]+(\.[0-9]+)?(\+|\*|\-|\/))+\-?[0-9]+(\.[0-9]+)?\"?",string):
        return True
    else:
        return False

#Returns true for expressions that may (or may not) contain variables
def isVariableExpression(string):
    if re.match(r"\"?(((\-?[0-9]+(\.[0-9]+)?)|([A-z_][A-z0-9_]+]))(\+|\*|\-|\/))+(((\-?[0-9]+(\.[0-9]+)?)|([A-z_][A-z0-9_]+]))\"?",string):
        return True
    else:
        return False
...
if __name__ == "__main__":
    for i, arg in enumerate(sys.argv[1:]):
        if isValidExpression(arg):
            r1 = EvaluateExpression(arg)
            r2 = eval(arg)
            print("Result: ",r1," (",r2,")")
        else:
            print("Not a valid expression (",arg,")")
